...
class ComponentZero(SiteNewsletterSignupValidatorComponent):
    ...
    @staticmethod
    def is_newsletter_popup_form(event: "pd.Series[Any]") -> bool:
        """
    ...
    def validate(self, event: "pd.Series[Any]") -> bool:
        return (
            self.has_data(event)
            and self.has_email_input(event)
            and (self.is_newsletter_inline_form(event) or self.is_newsletter_popup_form(event))
        )


COMPONENT_ZERO = ComponentZero(effective_starting=TIMESTAMP_POSIX)

# When OpenVallejo rules change, add a new component here (create a new child class of
